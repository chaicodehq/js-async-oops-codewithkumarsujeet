...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    const isResidentExist = this.#residents.some(
  ...
  removeResident(name, passcode) {
    ...
    const isResidentExist = this.#residents.find(
      (resident) => resident.name === name,
    );
    if (!isResidentExist) {
      return { success: false, message: "Resident nahi mila!" };
    }
    const index = this.#residents.indexOf((resident) => resident.name === name);
    if (index !== -1) {
